...
4. **ローカルIPアドレスの自動検出**:
   上記の方法で設定が見つからない場合、MCPサーバーはシステムのネットワークインターフェースから非ループバックIPv4アドレスを検出し、最初に見つかったアドレスに対してポート8765で接続を試みます。
   - 複数のネットワークインターフェースがある場合、最初に見つかったIPv4アドレスを使用します
   - コンソールログには検出されたすべてのIPアドレスが表示されます
